import 'package:flutter/material.dart';

const largeTextStyle = TextStyle(fontSize: 24.0);
const regularBoldStyle = TextStyle(fontSize: 20.0, fontWeight: FontWeight.w700);
const regularBoldStyleUnderline = TextStyle(
    fontSize: 20.0,
    fontWeight: FontWeight.w700,
    decoration: TextDecoration.underline);
const regularTextStyle = TextStyle(fontSize: 20.0);
const regularLightStyle =
    TextStyle(fontSize: 20.0, fontWeight: FontWeight.w100);
const regularHintTextStyle = TextStyle(fontSize: 20.0, color: Colors.grey);
const docTextStyle = TextStyle(fontSize: 16.0);

const containerXSPadding = EdgeInsets.all(4.0);
const containerSmallPadding = EdgeInsets.all(8.0);
const containerPadding = EdgeInsets.all(16.0);
const containerLargePadding = EdgeInsets.all(24.0);
const margin = Padding(padding: EdgeInsets.all(6.0));
const bottomBlank = Padding(
  padding: EdgeInsets.all(30.0),
);

const hnbPlaceholder = "x x x x";
