...
| 10 | 四子棋 (Connect Four) | 2026.5.18 | v7.0.0 |
...
- 当前最近推出：`connectFour`（2026.5.18）、`fishing`（2026.5.3）
...
AI位于`lib/games/connect_four/ai/`:
- `connect_four_ai.dart` - AI基类
- `easy_ai.dart` - 简单难度（随机+阻断）
- `medium_ai.dart` - 中等难度（Minimax+AlphaBeta 4层）
- `hard_ai.dart` - 困难难度（AlphaBeta 6层+启发式排序+时间控制）
...
**AI难度差异:**
- **Easy**: 仅检测即时获胜和阻断对手获胜，其余随机选择
- **Medium**: 4层Minimax+AlphaBeta剪枝，中心列优先，评估函数含连线权重
- **Hard**: 6层AlphaBeta剪枝+迭代加深+启发式列排序+威胁检测+时间预算控制（1.8秒）

**统一调用契约:**
- 输入: ConnectFourState、难度枚举、可选时间限制
- 输出: 最佳落子列索引（0-5）
- 中断: 支持取消信号，返回已缓存最优步
- 性能: 简单AI < 0.1秒，困难AI上限2秒
